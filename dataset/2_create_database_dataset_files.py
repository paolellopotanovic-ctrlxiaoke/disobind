"""
【中文解析-模块总览】
- 中心功能：2_create_database_dataset_files.py 属于 Disobind 数据流水线脚本，用于数据抓取/清洗/映射/导出中的一个环节。
...
class parse_pdbs_for_idrs():
	...
	def remove_missing_residues( self, mapped_uni_pos, mapped_pdb_pos ):
		...
		for idx in range( len( pdb_pos_no_nulls ) ):
			# Note: this implementation may miss some fragments in case a <min_len fragment is in the middle. Fix in a future version. 
